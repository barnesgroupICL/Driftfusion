...
% v4 Calculated on the whole mesh, uses jn instead of js
...
%     jns1 = jn(p1, par.pcum0(al-1)+1);
%     jps1 = jp(p1, par.pcum0(al-1)+1);
%     jns2 = jn(p1, par.pcum0(al+1)+1);
%     jps2 = jp(p1, par.pcum0(al+1)+1);  
    ...
    ps1_ana(i,:) = ps1.*exp(beta.*xprime_p) + jp(p1,:)./(beta.*muh.*Vt).*(1-exp(beta.*xprime_p))...
        - (r./(beta.^2.*muh.*Vt)).*(1-exp(beta.*xprime_p)+ beta.*xprime_p.*exp(beta.*xprime_p));
    ...
    ns1_ana(i,:) = ns1.*exp(alpha.*xprime_n) + jn(p1,:)./(alpha.*mue.*Vt).*(1-exp(alpha.*xprime_n))...
        - (r./(alpha.^2.*mue.*Vt)).*(1-exp(alpha.*xprime_n)+ alpha.*xprime_n.*exp(alpha.*xprime_n));
