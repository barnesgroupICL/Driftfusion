function [Voc, deltaV, monoexp_lin, monoexp_tau, biexp_lin_fast, biexp_tau_fast, biexp_lin_slow, biexp_tau_slow] = TPV_ana(sol_TPV, do_graphics, biexp_initial_guess_lin_fast, biexp_initial_guess_tau_fast, biexp_initial_guess_lin_slow, biexp_initial_guess_tau_slow)
...
Voct = dfana.calcVQFL(sol_TPV);
Voc = min(Voct(1),Voct(end));
V_minusBaseline = Voct - Voc;
par = sol_TPV.par;
...
[deltaV, peakIndex] = max(V_minusBaseline);
...
t_afterPeak = sol_TPV.t(peakIndex:end);
t_afterPeakZero = t_afterPeak - t_afterPeak(1);
...
monoexp_initial_guess = [deltaV, t_afterPeakZero(monoexp_initial_guess_tau_index)]; % give approximate values for starting the fit
% figure(1)
% plot(t_afterPeakZero, V_afterPeak, '.')
% hold on
% plot(sol_TPV.t-t_afterPeak(1), Voct, '.')
% plot(t_afterPeakZero,F(monoexp_initial_guess,t_afterPeakZero))

fit_monoexp = fitnlm(t_afterPeakZero', V_afterPeak, F, monoexp_initial_guess); % fitting
...
monoexp_y_predicted = predict(fit_monoexp, t_afterPeakZero'); % generate y data for plotting the fit result
...
lb = [deltaV/100,par.t0,deltaV/100,par.t0];
ub = [1,20*par.tmax,1,20*par.tmax];
...
if ~exist('biexp_initial_guess_lin_fast', 'var') || biexp_initial_guess_lin_fast == 0
    ...
    biexp_initial_guess_tau_slow = t_afterPeakZero(biexp_initial_guess_tau_slow_index) * 10;
    ...
    V_afterPeak_minusSlow = V_afterPeak - biexp_initial_guess_lin_slow * exp(-t_afterPeakZero / biexp_initial_guess_tau_slow);
    ...
    biexp_initial_guess_tau_fast = t_afterPeakZero(biexp_initial_guess_tau_fast_index);
...
problem = createOptimProblem('lsqcurvefit','x0',biexp_initial_guess,'objective',F2,...
    'lb',lb,'ub',ub,'xdata',t_afterPeakZero,'ydata',V_afterPeak);
...
%if biexp_success
    ...
    biexp_y_predicted = F2(xmulti, t_afterPeakZero'); % generate y data for plotting the fit result
%else
...
%% plot figures
if do_graphics
    figure('Name', ['Single TPV at light intensity ' num2str(sol_TPV.par.int1)], 'NumberTitle', 'off');
        hold off
        semilogy(sol_TPV.t, V_minusBaseline, '.');
        hold on
