function compare_carrier_interfaces_X(sol, tarr)
...
% jp(x), jns and jps
% vX As with 8.2 but carrier densities calculated on whole mesh, fluxes on
% half mesh
% TARR = array of time points
plotswitch = 1;     % 1 = plot n(x), p(x), 2 = plot ns, ps
...
for i = 1:length(t)
    jn(i,:) = [j.n(i,:), NaN];     % Add point to prevent fail when index reaches end
    jp(i,:) = [j.p(i,:), NaN];     % Add point to prevent fail when index reaches end
end
...
if plotswitch ==2
    ...
    for i = 1:length(tarr)
        ...
        subplot(1,2,1)
        semilogy(x*1e7, n(pnt, :), x*1e7, p(pnt, :),...
            x*1e7, ns1_ana(i,:), 'k-.', x*1e7, ps1_ana(i,:), 'k--')
        hold on
    end
    hold off
    title('Interface 1')
    ...
    for i = 1:length(tarr)
        ...
        subplot(1,2,2)
        semilogy(x*1e7, n(pnt, :), x*1e7, p(pnt, :),...
            x*1e7, ns2_ana(i,:), 'k-.', x*1e7, ps2_ana(i,:), 'k--')
        hold on
    end
    hold off
    title('Interface 1')
