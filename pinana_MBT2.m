function [Vapp_arr, Jtot] = pinana_MBT(varargin)

% This version analysis remote injection barrier heights i.e. Fermi level
% wrt the band energy at the otherside of the interface.
...
% Vrec
Vrec1p_0 = Efp_eq(1, pcum(2)-1) - Evb_eq(1, pcum(3));
Vrec1n_0 = Ecb_eq(1, pcum(3))- Efn_eq(1, pcum(2)-1);
Vrec2p_0 = Efp_eq(1, pcum(4)-1) - Evb_eq(1, pcum(5));
Vrec2n_0 = Ecb_eq(1, pcum(5))- Efn_eq(1, pcum(4)-1);
VRec0_verf = Vrec1n_0 + Vrec2p_0;

Vrec1p = Vrec1p_0-(Efp(:, pcum(2)-1) - Evb(:, pcum(3)));
Vrec1n = Vrec1n_0-(Ecb(:, pcum(3))- Efn(:, pcum(2)-1));
Vrec2p = Vrec2p_0-(Efp(:, pcum(4)-1) - Evb(:, pcum(5)));
Vrec2n = Vrec2n_0-(Ecb(:, pcum(5))- Efn(:, pcum(4)-1));

Vrec1p_rev = Vrec1p_0-(Efp_rev(:, pcum(2)-1) - Evb_rev(:, pcum(3)));
Vrec1n_rev = Vrec1n_0-(Ecb_rev(:, pcum(3))- Efn_rev(:, pcum(2)-1));
Vrec2p_rev = Vrec2p_0-(Efp_rev(:, pcum(4)-1) - Evb_rev(:, pcum(5)));
Vrec2n_rev = Vrec2n_0-(Ecb_rev(:, pcum(5))- Efn_rev(:, pcum(4)-1));
