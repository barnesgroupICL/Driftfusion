function [fit_coeff, fit_idrift_coeff, subtracting_n_t, subtracting_n_intr_t, subtracting_n_contacts_t, subtracting_a_t] = ISwave_single_analysis(sol_i_Int_ISwave)
...
%     plot(s.t(2:end), subtracting_n_t);
    ...
    plot(s.t(2:end), subtracting_a_t);
    plot(fit_t, s.params.J_func(fit_coeff, fit_t), 'g')
    plot(fit_t, J_outphase, 'r')
    legend_array = ["Total J", "Displacement J", "Charges intrinsic", "Charges contacts", "Displaced ions", "Fit", "Out of Phase Current"];
