...
% this instance I am calling the object PAR.
...
par_base = pc('input_files/mapi_tio2_bilayer.csv');
...
soleq_base = equilibrate(par_base);

%% Do the SPV procedure
% SOLEQ contains 2 solutions, with and without ions. Here we will use
% the solution SOLEQ.ION as the input to DOSPV which can be found in
...
spvsol_base = doSPV(soleq_base.ion, 0.2, 1, 200, 10, 1e6, 0);

%% Set surface rec velocities to 1 on LHS
par_snl_spl1 = par_base;
par_snl_spl1.sn_l = 1;
par_snl_spl1.sn_r = 1;
par_snl_spl1 = refresh_device(par_snl_spl1);

soleq_snl_spl1 = equilibrate(par_snl_spl1);
spvsol_snl_spl1 = doSPV(soleq_snl_spl1.ion, 0.2, 1, 200, 10, 1e6, 0);
...
spvdat_base = spvana(spvsol_base);

spvdat_snl_spl1 = spvana(spvsol_snl_spl1);
...
dfplot.ELxnpxacx(spvsol_base.ill);
...
dfplot.Vx(spvsol_base.ill, [0,1,2,4,8]);
