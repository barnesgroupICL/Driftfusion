function IS_results = IS_script_nonparallel(structs, startFreq, endFreq, Freq_points, deltaV, sequential, frozen_ions, demodulation, do_graphics, save_solutions)
%IS_SCRIPT_NONPARALLEL - Simulates impedance spectroscopy at various frequencies on many provided solutions
...
% The fundamental difference from IS_script is the lack of parfor
% for parallel computation. This allows more flexibility at the cost of a
% more time demanding simulation.
%
% Syntax:  IS_results = IS_script_nonparallel(structs, startFreq, endFreq, Freq_points, deltaV, sequential, frozen_ions, demodulation, do_graphics, save_solutions)
%
% Inputs:
...
%   SAVE_SOLUTIONS - logical, defines whether to save in volatile base
...
%   IS_vapp = IS_script_nonparallel(genVappStructs(soleq.ion, 0:0.2:1, true), 1e9, 1e-2, 56, 2e-3, false, false, true, false, true)
...
for i = 1:length(structs(1, :))
    ...
    s.par.figson = 0;
    if sequential
        % do not repeat simulation for reaching oscillating stability
        stability_timefraction = 1;
    else
        % repeat simulation for reaching oscillating stability
        stability_timefraction = 0.5;
    ...
    for j = 1:length(Freq_array)
        ...
        s_IS = doIS_EA(struct_start, deltaV,...
            Freq_array(j), periods, tpoints_per_period, stability_timefraction, tempRelTol); % do IS
        ...
        if coeff.Jtot(3) < 0.006 || coeff.Jtot(3) > pi/2 - 0.006
            disp([mfilename ' - Int: ' num2str(Int_array(i)) '; Vdc: ' num2str(Vdc_array(i)) ' V; Freq: ' num2str(Freq_array(j)) ' Hz; Phase is ' num2str(rad2deg(coeff.Jtot(3))) ' degrees, increasing solver accuracy and calculating again'])
            % decrease tollerance
            tempRelTol = tempRelTol / 100;
            % if just the initial solution, non-stabilized, is requested, do
            % not start from oscillating solution
            if sequential
                struct_temp = struct_start; % strictly use the last point from previous cycle
            else
                struct_temp = s_IS; % the last point of the oscillating solution, better starting point
            end
        ...
        if coeff.Jtot(3) < 0 || coeff.Jtot(3) > pi/2
            disp([mfilename ' - Int: ' num2str(Int_array(i)) '; Vdc: ' num2str(Vdc_array(i)) ' V; Freq: ' num2str(Freq_array(j)) ' Hz; Phase is weird: ' num2str(rad2deg(coeff.Jtot(3))) ' degrees, confirming using alternative fitting method'])
        ...
        if coeff.Jtot(3) < 0 || abs(coeff.Jtot(3)) > pi/2
            disp([mfilename ' - Int: ' num2str(Int_array(i)) '; Vdc: ' num2str(Vdc_array(i)) ' V; Freq: ' num2str(Freq_array(j)) ' Hz; Phase is still weird: ' num2str(rad2deg(coeff.Jtot(3))) ' degrees, increasing solver accuracy and calculating again'])
            tempRelTol = tempRelTol / 100;
            % if just the initial solution, non-stabilized, is requested, do
            % not start from oscillating solution
            if sequential
                ...
                struct_temp = s_IS; % the oscillating solution, better starting point
            end
            s_IS = doIS_EA(struct_temp,...
                deltaV, Freq_array(j), periods, tpoints_per_period, stability_timefraction, tempRelTol); % do IS
            % repeat analysis on new solution
        ...
        if save_solutions % assignin cannot be used in a parallel loop, so single solutions cannot be saved
            sol_name = matlab.lang.makeValidName([structs{2, i} '_Freq_' num2str(Freq_array(j)) '_IS']);
            s_IS.par.figson = 1; % re-enable figures by default when using the saved solution, that were disabled above
            assignin('base', sol_name, s_IS);
        end
