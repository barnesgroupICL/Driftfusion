...
par_tio2_highepp = pc('Input_files/spiro_mapi_tio2_mirror_highepp.csv');

%% Find the equilibrium solutions
soleq_tio2_highepp = equilibrate(par_tio2_highepp);
...
sol_CV_100mVs_highepp = doCV(soleq_tio2_highepp.ion, 1, 0, -1.2, 0.2, 100e-3, 1, 281);
sol_CV_200mVs_highepp = doCV(soleq_tio2_highepp.ion, 1, 0, -1.2, 0.2, 200e-3, 1, 281);
sol_CV_400mVs_highepp = doCV(soleq_tio2_highepp.ion, 1, 0, -1.2, 0.2, 400e-3, 1, 281);

%% plot the current voltage curve
dfplot.JtotVapp_mirror(sol_CV_100mVs_highepp, 0)
hold on
dfplot.JtotVapp_mirror(sol_CV_200mVs_highepp, 0)
hold on
dfplot.JtotVapp_mirror(sol_CV_400mVs_highepp, 0)
hold off
ylim([-30e-3,10e-3])
xlim([-0.2, 1.2])
