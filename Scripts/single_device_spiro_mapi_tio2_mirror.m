...
par_tio2_mirror = pc('Input_files/spiro_mapi_tio2_mirror.csv');

%% Find the equilibrium solutions
soleq_tio2_mirror = equilibrate(par_tio2_mirror);
...
sol_CV_100mVs_mirror = doCV(soleq_tio2_mirror.ion, 1, 0, -1.2, 0.2, 100e-3, 1, 281);
sol_CV_200mVs_mirror = doCV(soleq_tio2_mirror.ion, 1, 0, -1.2, 0.2, 200e-3, 1, 281);
sol_CV_400mVs_mirror = doCV(soleq_tio2_mirror.ion, 1, 0, -1.2, 0.2, 400e-3, 1, 281);

%% plot the current voltage curve
dfplot.JtotVapp_mirror(sol_CV_100mVs_mirror, 0)
hold on
dfplot.JtotVapp_mirror(sol_CV_200mVs_mirror, 0)
hold on
dfplot.JtotVapp_mirror(sol_CV_400mVs_mirror, 0)
hold off
ylim([-30e-3,10e-3])
xlim([-0.2, 1.2])
...
dfplot.ELxnpxacx(sol_CV_100mVs_mirror, 10)
