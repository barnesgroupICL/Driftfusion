function [rhomat, Fmat, Phimat, Q, tout] = depletion_approx_modelX_Vjump_check(par, tmax, Vstart, Vjump, tpoints, tarr, figson)
% Philip Calado 2019
...
switch par.N_ionic_species
    case 1
        R = (par.dcell(par.active_layer)-2*w2)./(par.e.*par.Ncat(par.active_layer).*par.mucat(par.active_layer));
    case 2
        R = (par.dcell(par.active_layer)-2*w2)./(par.e.*(par.Ncat(par.active_layer).*par.mucat(par.active_layer) + par.Nani(par.active_layer).*par.muani(par.active_layer)));
end
% Capacitances
Cap.C1 = epp1/w1;
Cap.C2 = epp2/w2;
...
Cap.Cg_dash = epp2/(par.d_active-2*w2);
Cap.CT_a = 1/(1/Cap.Cint1 + 1/Cap.Cint2);
Cap.CT_b = 1/(1/Cap.Cint1 + 1/Cap.Cg_dash + 1/Cap.Cint2);
Cap.CT_c = 1/(1/Cap.Cint1 + 1/Cap.Cint2) + Cap.Cg_dash;
tau.RC_g = 1/((2/(R*Cap.C2)) + (((1/(Cap.C1*R)) + (1/(Cap.C3*R)))/((Cap.Cg_dash/Cap.C1) + (Cap.Cg_dash/Cap.C3) +1)));
...
Vapp = (Vstart + Vjump)*ones(tpoints);
V = Vbi - Vapp;
Chi = 1/(N1*epp1) + 1/(N3*epp3);
...
% d1+w2 < x < d2-w2 (Region 4)
    function dQdt = odefun(t, Q)
        % Second term on rhs is the Potential across the perovskite:
        Vr = (Vbi-(Vstart + Vjump));
        deltaV = ((1/(epp2*Chi))*((q^2*(d1-d2)^2 + 2*q*epp2*Chi*(Vr*epp2 - Q^2/(q*N2) - Q*(d1-d2)))^(1/2) + q*(d1-d2)) - Q)*(d2-d1-2*w2)/epp2;
        
        dQdt = (1/R).*deltaV;
        
      
%        dQdt = (1/R)*(((N1*epp1*Q + N3*epp3*Q)*(2*Q + N2*d1*q - N2*d2*q))/(N2*epp2*q*(N1*epp1 + N3*epp3))...
%             - ((N3*epp1*epp3*((N1*q*(2*N1*(Vbi-(Vstart + Vjump))*epp1*epp2^2 + 2*N3*(Vbi-(Vstart + Vjump))*epp2^2*epp3...
%             - 2*N1*d1*epp1*epp2*Q + 2*N1*d2*epp1*epp2*Q - 2*N3*d1*epp2*epp3*Q + 2*N3*d2*epp2*epp3*Q - (2*N1*epp1*epp2*Q^2)/(N2*q)...
%             - (2*N3*epp2*epp3*Q^2)/(N2*q) + N1*N3*d1^2*epp1*epp3*q + N1*N3*d2^2*epp1*epp3*q...
%             - 2*N1*N3*d1*d2*epp1*epp3*q))/(N3*epp1*epp3))^(1/2) + N1*N3*d1*epp1*epp3*q - N1*N3*d2*epp1*epp3*q)...
%             *(2*Q + N2*d1*q - N2*d2*q))/(N2*epp2^2*q*(N1*epp1 + N3*epp3)));
    end
    ...
    function Fsol = getF
        ...
        for ii = 1:length(x)
            if x(ii) <= d1 - (w1 + wapp1)
                Fsol(ii) = 0;
            elseif x(ii) <= d1 && d1 - (w1 + wapp1) < x(ii)
                Fsol(ii) = q*N1*(-x(ii)+d1-(w1 + wapp1))/epp1;
            elseif x(ii) <= d1 + w2 && d1 < x(ii)
                Fsol(ii) = q*(N2*(x(ii)-d1) -N1*(w1 + wapp1))/epp2;
            elseif d1 + w2 < x(ii) && x(ii) <= d2 - w2
                Fsol(ii) = q*(N2*w2 - N1*(w1 + wapp1))/epp2;
            elseif x(ii) <= d2 && d2 - w2 < x(ii)
                Fsol(ii) = q*(N2*(d2-x(ii))- N1*(w1 + wapp1))/epp2;
            elseif x(ii) <= d2 + (w3 + wapp3) && d2 < x(ii)
                Fsol(ii) = q*(N3*(x(ii)-d2)- N1*(w1 + wapp1))/epp3;
            elseif d2 + (w3 + wapp3) < x(ii) && x(ii) <= d3
                Fsol(ii) = q*(N3*(w3 + wapp3) - N1*(w1 + wapp1))/epp3;
            end
        end
    end
    ...
    function Phisol = getPhi
        ...
        for ii = 1:length(x)
            if x(ii) <= d1 - (w1 + wapp1)
                Phisol(1,ii) = 0;
            elseif x(ii) <= d1 && d1 - (w1 + wapp1) < x(ii)
                Phisol(1,ii) = q*N1*(x(ii)-d1+(w1 + wapp1)).^2/(2*epp1);
            elseif x(ii) <= d1 + w2 && d1 < x(ii)
                Phisol(1,ii) = q*(-N2.*(x(ii)-d1).^2 + 2*(x(ii)-d1)*N1.*(w1 + wapp1))/(2*epp2) + q.*N1.*(w1 + wapp1).^2/(2*epp1);
            elseif d1 + w2 < x(ii) && x(ii) <= d2 - w2
                Phisol(1,ii) = (q/(2*epp2))*(2*(N1.*(w1 + wapp1) - N2.*w2).*(x(ii)-d1) + N2.*w2.^2) + q.*N1.*(w1 + wapp1).^2/(2*epp1);
            elseif x(ii) <= d2 && d2 - w2 < x(ii)
                Phisol(ii) = (q/(2*epp2))*((N2.*(x(ii)-d2+w2).^2)+ 2*(N1.*(w1 + wapp1) - N2.*w2).*(x(ii)-d1) + N2.*w2.^2) + q.*N1.*(w1 + wapp1).^2/(2*epp1);
            elseif x(ii) <= d2 + (w3 + wapp3) && d2 < x(ii)
                Phisol(ii) = (q/(2*epp3)).*(-N3.*(x(ii)-d2).^2  + 2*N1.*(w1 + wapp1).*(x(ii)-d2)) +...
                    (q/(2*epp2))*(2*(N1.*(w1 + wapp1)- N2*w2)*(d2-d1) +  2*N2.*w2^2) + q.*N1.*(w1 + wapp1).^2/(2*epp1);
            elseif d2 + (w3 + wapp3) < x(ii) && x(ii) <= d3
                Phisol(ii) = (q/(2*epp3))*(2*(N1.*(w1 + wapp1) - N3.*(w3 + wapp3)).*(x(ii)- d2) + N3.*(w3 + wapp3).^2) +...
                    (q/(2*epp2))*(2*(N1.*(w1 + wapp1)- N2*w2)*(d2-d1) +  2*N2.*w2^2) + q.*N1.*(w1 + wapp1).^2/(2*epp1);
            end
        end
    end
...
for i = 1:length(t)
    ...
    Qapp = (1/(epp2*Chi))*((q^2*(d1-d2)^2 + 2*q*epp2*Chi*(V(i)*epp2 - Q(i)^2/(q*N2) - Q(i)*(d1-d2)))^(1/2) + q*(d1-d2)) - Q(i);
    wapp1 = Qapp/(q*N1);
    wapp3 = Qapp/(q*N3);
