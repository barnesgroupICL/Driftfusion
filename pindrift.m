...
if length(varargin) == 0 || length(varargin) >= 1 && max(max(max(varargin{1, 1}.sol))) == 0
    ...
    elseif  x >= (p.tp - p.wp) && x < p.tp
        ...
        u0 = [p.N0*exp((p.Efnside + p.EA + p.q*((((p.q*p.NA)/(2*p.eppi*p.epp0))*(x-p.tp+p.wp)^2)))/(p.kB*p.T));                            %ni*exp((Efnside - (-q*((((q*NA)/(2*eppp))*(x-tp+wp)^2))))/(kB*T));
              p.N0*exp(-(p.q*((((p.q*p.NA)/(2*p.eppi*p.epp0))*(x-p.tp+p.wp)^2)) + p.EA + p.Eg + p.Efpside)/(p.kB*p.T));
              p.NI;
    ...
    elseif  x > (p.tp+p.ti) && x <= (p.tp + p.ti + p.wn)
        ...
        u0 = [p.N0*exp((p.Efnside + p.EA + p.q*((((-(p.q*p.ND)/(2*p.eppi*p.epp0))*(x-p.ti-p.tp-p.wn)^2) + p.Vbi)))/(p.kB*p.T));
              p.N0*exp(-(p.q*((((-(p.q*p.ND)/(2*p.eppi*p.epp0))*(x-p.ti-p.tp-p.wn)^2) + p.Vbi)) + p.EA + p.Eg + p.Efpside)/(p.kB*p.T));
              p.NI;
