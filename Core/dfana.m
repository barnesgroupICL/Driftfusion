classdef dfana
    ...
    methods (Static)
        ...
        function r = calcr(sol)
            ...
            eppmat = repmat(dev.epp, length(t), 1);
            nimat = repmat(dev.ni, length(t), 1);
            ni_srhmat = repmat(dev.ni_srh, length(t), 1);
            ...
            r.srh = ((n.*p - ni_srhmat.^2)./((taunmat.*(p+ptmat)) + (taupmat.*(n+ntmat))));
            r.tot = r.btb + r.srh;
        end
        ...
        function r = calcr_ihalf(sol)
            ...
            ni_srhmat = repmat(devihalf.ni_srh, length(t), 1);
            ...
            r.srh = ((n_ihalf.*p_ihalf - ni_srhmat.^2)./((taunmat.*(p_ihalf+ptmat)) + (taupmat.*(n_ihalf+ntmat))));
            r.tot = r.btb + r.srh;
        end
