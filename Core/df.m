...
% S = Source terms
    function [C,F,S] = dfpde(x,t,u,dudx)   
        ...
        % Source terms
        S_electron = g - radset*Bdir(i)*((n*p)-(ni(i)^2)) - SRHset*(((n*p)-ni(i)^2)/((taun(i)*(p+pt(i)))+(taup(i)*(n+nt(i)))));
        S_hole     = g - radset*Bdir(i)*((n*p)-(ni(i)^2)) - SRHset*(((n*p)-ni(i)^2)/((taun(i)*(p+pt(i)))+(taup(i)*(n+nt(i)))));
        S_cation   = 0;
