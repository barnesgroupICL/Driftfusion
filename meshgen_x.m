...
case 1
    ...
    x = linspace(0,p.dcum(end),p.parr(1)+p.parr(2)+p.parr(3));

% Linearly spaced, more points at interfaces   
case 2
   
    x = [linspace(0, p.dcum(1)-p.dint, p.parr(1)),...
        ...
        linspace(p.dcum(2)+p.dint, p.dcum(end), p.parr(3));];
...
case 3
    ...
    x = [linspace(0, p.dcum(1)-p.dscr, p.parr(1)),...
        ...
        linspace(p.dcum(2)+p.dscr, p.dcum(end), p.parr(3));];
...
case 4
    ...
    x = [linspace(0, p.te, p.pepe),...
        ...
        linspace(p.dcum(2)+p.dscr, p.dcum(end)-p.te-p.deltax, p.parr(3)),...
        linspace(p.dcum(end)-p.te, p.dcum(end), p.pepe);]; 
...
case 5
    ...
    x = [linspace(0, p.telp, p.pelp),...
        ...
        linspace(p.telp+p.dcum(2)+p.dscr, p.dcum(end)-p.teln, p.parr(3)),...
        linspace(p.dcum(end)-p.teln+(p.teln/p.peln), p.dcum(end), p.peln);];
...
case 7
    ...
    x = [linspace(0, p.telp-p.tjn, p.pelp),...
        ...
        linspace(p.telp+p.dcum(2), p.telp+p.dcum(2)+p.dint-(p.dint/p.pint), p.pint),...
        linspace(p.telp+p.dcum(2)+p.dint, p.telp+p.dcum(2)+p.dscr-(p.dscr/p.pscr), p.pscr),...
        linspace(p.telp+p.dcum(2)+p.dscr, p.dcum(end)-(p.teln+p.tjn), p.parr(3)),...
        linspace(p.dcum(end)-(p.teln+p.tjn-(p.tjn/p.pjn)), p.dcum(end)-(p.teln-p.tjn+(p.tjn/p.pjn)), p.pjn),...
        linspace(p.dcum(end)-(p.teln-p.tjn), p.dcum(end), p.peln);];
