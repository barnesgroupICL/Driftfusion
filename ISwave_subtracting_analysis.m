...
i_delta_matrix(:, x_center_index:end) = -i_delta_matrix(:, x_center_index:end);
% dividing by two for not counting twice each ion (once from where it left and once for where it arrived)
i_delta_t = transpose(trapz(s.x, i_delta_matrix, 2) / 2);


% sum of the ambsolute delta ions profile over thickness, obtain a point
